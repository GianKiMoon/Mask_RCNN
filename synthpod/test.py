...
img, ann_m, ann_i, ann_u, ann_v = util.load_test_img(55)#9
...
res_argmax = np.argmax(i, 3)
res_argmax = np.squeeze(res_argmax)
res_argmax = res_argmax.astype(np.float32)
res_argmax = util.give_color_to_seg_img(res_argmax, 25)
res_argmax = cv2.resize(res_argmax, (256, 256), interpolation=0)
numpy_horizontal = np.hstack((cv2.resize(util.give_color_to_seg_img(np.argmax(np.squeeze(ann_i), 2), 25), (256, 256), interpolation=0),
